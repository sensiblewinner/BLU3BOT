...
// ====== BLU3BOTBOT - ULTIMATE CLEAN EDITION (SPEED OPTIMIZED) ======
...
// New: BLU3BOT session format support (BLU3BOT:eyJ...)
// New: Professional success messaging like BLU3BOT
...
const GROUP_NAME = 'BLU3BOT Community';
...
║   📱 Session Support: BLU3BOT: format & Base64
    ...
    async sendWelcomeMessage(sock, userJid) {
        ...
        try {
            await sock.sendMessage(userJid, {
                text: `🎉 *WELCOME TO BLU3BOT!*\n\n` +
                      `Thank you for connecting with BLU3BOT! 🤖\n\n` +
                      `✨ *Features Available:*\n` +
...
// ====== SESSION ID PARSER (FROM BLU3BOT) ======
function parseBLU3BOTSession(sessionString) {
    try {
        ...
        // Check if it starts with BLU3BOT:
        if (cleanedSession.startsWith('BLU3BOT:')) {
            UltraCleanLogger.info('🔍 Detected BLU3BOT: prefix');
            const base64Part = cleanedSession.substring(9).trim();
            
            if (!base64Part) {
                throw new Error('No data found after BLU3BOT:');
            }
...
async function authenticateWithSessionId(sessionId) {
    try {
        ...
        // Parse the session
        const sessionData = parseBLU3BOTSession(sessionId);
        ...
        if (error.message.includes('BLU3BOT')) {
            UltraCleanLogger.info('📝 Expected format: BLU3BOT:{base64_data}');
...
class LoginManager {
    ...
    async selectMode() {
        console.log(chalk.yellow('\n🐺 BLU3BOT v' + VERSION + ' - LOGIN SYSTEM'));
    ...
    async sessionIdMode() {
        ...
        if (!sessionId || sessionId.trim() === '') {
            ...
            if (input.trim() === '1') {
                sessionId = await this.ask('Paste your Session ID (BLU3BOT:... or base64): ');
                if (!sessionId || sessionId.trim() === '') {
    ...
    try {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                // Send professional success message like BLU3BOT
                setTimeout(async () => {
                    try {
                        const ownerJid = sock.user.id;
                        const cleaned = jidManager.cleanJid(ownerJid);
...
async function handleSuccessfulConnection(sock, loginMode, loginData) {
    ...
    console.log(chalk.greenBright(`
╔══════════════════════════════════════════════════════════════════════╗
║                    🐺 ${chalk.bold('BLU3BOT ONLINE')} - v${VERSION} (PREFIXLESS & MEMBER DETECTION) ║
╠══════════════════════════════════════════════════════════════════════╣
...
async function main() {
    try {
        ...
        UltraCleanLogger.info(`🔐 Session ID support: ✅ ENABLED (BLU3BOT: format)`);
